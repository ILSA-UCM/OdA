...
if (!$visit->options->tieneAcceso("ls",new ClsNumericData())) $visit->options->sinAcceso();
...
// alfredo 140716  $session->lsnumeric_data= $visit->util->getUrlQuery("",$visit->util->getQueryString());
$_SESSION["lsnumeric_data"]= $visit->util->getUrlQuery("",$visit->util->getQueryString());
 ...
 * Descomentar y sustituir para Metodo de obtención de filas directo sin filtros.
 $count = $visit->dbBuilder->getNumericDataCount(); 
 $inicio = ( $npag - 1 ) * $visit->options->paginacion + 1;
 $filas = $visit->dbBuilder->getNumericDataLimit($inicio -1 ,$visit->options->paginacion);
*/

$numericData = new ClsNumericData();


if (trim($ordenar)!="") $numericData->_orderby = $ordenar;
$count = $visit->dbBuilder->getTablaFiltradaCount($numericData); 
$inicio = ( $npag - 1 ) * $visit->options->paginacion + 1;
if ($count==0) $inicio=0;
$filas = $visit->dbBuilder->getTablaFiltradaLimit($numericData, $inicio - 1 ,$visit->options->paginacion);
	...
	<FORM METHOD=GET ACTION="<?= $visit->util->getNombreArchivo( $visit->util->getScriptName() ) ?>" name="formlistado" style="display:inline;">
				...
				<TD width="200" valign="bottom" align="left">
					<TABLE width="200" border="0" cellpadding="0" cellspacing="0"  background="img/filtros_tit_fondo.gif" style="border-left:1px solid #B1B1B1;border-right:1px solid #B1B1B1;border-top:1px solid #B1B1B1;" > 		
						<TR>
							...
							<TD valign="middle" width="130"  nowrap><span class="titcuadro">Listado de numeric_data</span></TD>
		...
		<TABLE  border="0" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #CCCCCC;">
			<TR>
				...
				<a href="cm_form_numeric_data.php"><IMG SRC="img/boton_nuevo.gif" WIDTH="77" HEIGHT="21" BORDER="0" ALT=""></a>
				&nbsp;				
				</TD>
			</TR>
		</TABLE>
...
<TD width="13" class="<?= $lsregistros ?>"><a href="cm_form_numeric_data.php?id=<?= $filas[$i]->id ?>"><IMG SRC="img/ico_editar.gif" WIDTH="13" HEIGHT="16" BORDER="0" ALT=""></a></TD>
					<TD class="<?= $lsregistros ?>"><?= $filas[$i]->value ?></TD>
			<TD class="<?= $lsregistros ?>">
				<A HREF="do.php?op=mover_numeric_data&id=<?= $filas[$i]->id ?>&valor=1"><IMG SRC="img/flecha_up.gif" WIDTH="16" HEIGHT="16" BORDER="0" ALT=""></A>
				<A HREF="do.php?op=mover_numeric_data&id=<?= $filas[$i]->id ?>&valor=-1"><IMG SRC="img/flecha_down.gif" WIDTH="16" HEIGHT="16" BORDER="0" ALT=""></A>
			</td>
			<TD class="<?= $lsregistros ?>">		
				<A HREF="#" onclick="
				if (confirm('Seguro que desea eliminar el elemento?')) {
					window.location.href='do.php?op=eliminar_numeric_data&id=<?= $filas[$i]->id ?>';
				}
