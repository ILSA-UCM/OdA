...
<form name="formulario" action="do.php" method="POST" ENCTYPE="multipart/form-data" onsubmit="; return compruebaCampos()">
	...
	<TABLE  border="0" width="100%" cellpadding="0" cellspacing="10" style="border:1px solid #CCCCCC;">
			<TR>				
				...
				<TD align="center" width="34%">
					<!--  <A HREF="cm_ls_section_data.php" onclick="
						...
						"><IMG SRC="img/boton_volver2.gif" WIDTH="84" HEIGHT="21" BORDER="0" ALT="Guardar"></A> -->
							...
							<A HREF="<?= $_SESSION["ls_section_data"] ?>" onclick="
						var ir=false;
						if (revisarForm()) ir=true;
				...
				<!--  alfredo 140724 comenta elininar para que la opraci—n quede restringida a un solo formuario
				<TD align="right" width="33%">
						...
						************************************************* alfredo 140723  
						<A HREF="#" onclick="
							if (confirm('<?=utf8_encode("Seguro que desea eliminar el elemento?")?>')) {
								if (confirm('Se eliminar\u00e1 el contenido de esta secci\u00f3n de todos los objetos.\nEst\u00e1; seguro?')) {
						...
						<A HREF="#" onclick="
							...
							if (confirm('Seguro que desea eliminar el elemento?')) {
								if (confirm('Se va a eliminar el contenido de este elemento de TODOS los objetos.\n Seguro que quiere borrar TODOS esos datos?')) {
									window.location.href='do.php?op=eliminar_section_data&id=<?= $fila->id ?>';
									cerrar='<?$_SESSION["popup_section_data"]=true; ?>';
								}
							}
							event.returnValue=false;
							return false;
					...
					<? } else if($id!=""){ ?>
						...
						Este elemento no se puede borrar.<br>
	...
	<tr>
		...
		<TABLE  border="0" width="100%" cellpadding="0" cellspacing="10" style="border-left:1px solid #CCCCCC;border-right:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">
			<TR>				
				...
				<!-- alfredo 140723 comenta eliminar para restringirlo a un solo fromulario 
				<TD align="right" width="33%">
					&nbsp;
					<? if ($id!=""&&$id!=111&&$id!=112&&$id!=4) { ?>
						************************************************** alfredo 140723  <A HREF="#" onclick="
							if (confirm('Seguro que desea eliminar el elemento?')) {
								if (confirm('Se eliminar\u00e1 el contenido de esta secci\u00f3n de todos los objetos.\nEst\u00e1 seguro?')) {
									window.location.href='do.php?op=eliminar_section_data&id=<?= $fila->id ?>';
								}
