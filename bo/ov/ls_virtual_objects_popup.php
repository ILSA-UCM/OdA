...
$_SESSION["lsvirtual_object_popup"]= $visit->util->getUrlQuery("",$visit->util->getQueryString()); //alfredo 140801
...
<TABLE class="lstabla" width="100%"  cellpadding="0" cellspacing="0">
	...
	<TR>
		<TD width="30px" class="lscabecera" 
				<? if ($orden=="name") { ?>
	...
	<? if($idov!=$filas[$i]->id){ //alfredo 140802 ?>
		<tr>	
			<TD class="<?= $lsregistros ?>"><a href='#' <? if ($recOV=="N") { ?> onclick='window.open("ls_resources_popup.php?id=<?=$filas[$i]->id ?>&idov=<?=$idov?>&propios=S","","width=900,height=800,scrollbars=yes");return false;' <? } else { ?> onclick='asignarOV(<?= $filas[$i]->id?>);window.close();return false;' <? } ?>> OV  <?= $filas[$i]->id ?></a></TD>
