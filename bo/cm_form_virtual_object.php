...
<script>
	...
	function compruebaZip(){
		if (document.formulario.nombre_zip.value!=""){
			...
			//alert(extension);
			if (extension!="zip") {
				alert("El archivo a cargar no es vï¿½lido. Compruebe que su extensiï¿½n es .zip");
				return false;
			}
		}
		return true;
	...
	function compruebaZipHtml(){
		...
		if (extension!="zip" && extension!="html") {
			alert("El archivo a cargar no es vï¿½lido. Compruebe que su extensiï¿½n es .zip o .html");
			return false;
		}
...
<table width="100%" border="0" cellpadding="3" cellspacing="1"  style="border:1px solid #CCCCCC;">
	...
	while (list ($clave, $sectionData) = each ($filas)) { ?>		
		...
		if (!$visit->util->isInStr($caminoItemsStr,$idRecurso->id) ) { ?>
			...
			<td class="popuptitcampo" valign="top">
					...
					<a href='#' onclick='window.open("popup_form_section_data.php?idpadre=<?= $sectionData->id?>","","width=680,height=500,scrollbars=yes");return false' ><img src="img/ico_mas.gif" width="9" height="9" border="0" alt="Nueva Secciï¿½n"> </a><? } ?>
			...
			<? 
					...
					<td  class="popupcampo">
						...
						<a href="javascript:show_calendar('document.formulario.seccion_<?= $sectionData->id ?>', document.formulario.seccion_<?= $sectionData->id ?>.value);"><img src="../img/cal.gif" width="16" height="16" border="0" alt="Pinche aquï¿½ para introducir una fecha"></a>
					</td>
										...
										<tr>
											...
											<?	
												...
												<td  class="popupcampo">
													...
													<a href="javascript:show_calendar('document.formulario.seccion_<?=$sec->id?>_recurso_<?=$rec->id ?>', document.formulario.seccion_<?=$sec->id?>_recurso_<?=$rec->id ?>.value);"><img src="../img/cal.gif" width="16" height="16" border="0" alt="Pinche aquï¿½ para introducir una fecha"></a>
										...
										if ($sec2->nombre!="" && $visit->util->normalizeString($sec2->nombre)!="propietario") {
											...
											<tr>
												...
												<td  class="popupcampo">
													...
													<a href="javascript:show_calendar('document.formulario.seccion_<?=$sec2->id?>_nuevo', document.formulario.seccion_<?=$sec2->id?>_nuevo.value);"><img src="../img/cal.gif" width="16" height="16" border="0" alt="Pinche aquï¿½ para introducir una fecha"></a>
	...
	<!-- <tr>
		<td align='center' colspan='2'><a href='#' onclick='window.open("popup_form_section_data.php?idov=<?= $id?>&recOV=S","","width=680,height=500,scrollbars=yes");return false' >Nueva Secciï¿½n </a></td>
	</tr> -->
	</table>
			...
			<TR>				
				...
				<TD align="center" width="34%">
					<A HREF="<?= $session->lsvirtual_object ?>" onclick="
						var ir=false;
						if (revisarForm()) ir=true;
						else if (confirm('Ha modificado datos ï¿½Seguro que desea volver?')) ir=true;
						event.returnValue=ir;
						return ir;
...
<script>
	function comprobarRecurso()
	{
		...
		}else
		{
			...
			document.getElementById('error_archivo_recurso').innerHTML='Extensiï¿½n no vï¿½lida';
