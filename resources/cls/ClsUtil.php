...
function imprimirPaginacion($paginacion, $maxPaginasSiguientes, $count, $nPagActual, $urlName ) {
	if ($count==0) {
		...
		$paginas = $this->gerArrayPaginacion($paginacion, $maxPaginasSiguientes, $count, $nPagActual, $urlName );
		$valorFinal = $this->getValorFinalPaginacion($paginacion, $maxPaginasSiguientes, $count ,$nPagActual);
		// alfredo 140718  $valorInicial = $this->getValorInicialPaginacion($paginacion, $maxPaginasSiguientes, $count ,$nPagActual);
		$valorInicial = 1;
...
function construyeURLClasificacion($array_criterios,$idpadre,$idseccion){
	...
	if($array_criterios !=""){
		foreach($array_criterios as $key=>$value){
			if($key != "" && $value !=""){
				$value=str_replace("+", "xxjj", $value); //alfredo 180613
...
function getRequest2superGlobal() {  
// alfredo 140830
	$dict=$_SESSION['lsvirtual_object_busqueda'];
	return $dict;
}
	...
	*/
	...
	function mkdir($directorio) {		
		if (file_exists($directorio)) {
			...
			$this->copiarArchivo("../../bo/download/iconos/index.php", $directorio."/index.php");//alfredo 140913
		}
	}

function mkdirNoIndex($directorio) {		
		if (file_exists($directorio)) {
...
//			$visit->debuger->out($file);			
	...
	function getEnlaceFromMenu($fila, $menu){
		...
		if ($fila->tipo_contenido =="H") {
			...
			// alfredo 140905  $enlace = $_parenDir."/view/cm_view_virtual_object.php";
			 $enlace = $_parenDir."/view/cm_view_virtual_object.php?seleccion=1";
