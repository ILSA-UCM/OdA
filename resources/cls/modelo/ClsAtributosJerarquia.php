<?
class ClsAtributosJerarquia extends ClsModelo {
	var $id, $nombre, $tipo, $formato, $valor, $activo, $lang, $idlangprincipal, $es_jerarquia;

	function getNombreTabla() {
		return "atributos_jerarquia";
	}

	function getCamposUpdate() {
		return "nombre, tipo, formato, valor, activo, lang, idlangprincipal, es_jerarquia";
	}

	var $_orderby="activo, es_jerarquia, formato, idlangprincipal, lang, nombre, tipo, valor, id";
	...
	function getValoresUpdate() {
		...
		$res =	"nombre= ". $visit->util->getNullString( $this->nombre ) .",
				...
				es_jerarquia= ". $visit->util->getNullString( $this->es_jerarquia );
	...
	function getValoresInsert() {
		...
		$res =	$visit->util->getNullString( $this->nombre ) .",".
				...
				$visit->util->getNullString( $this->es_jerarquia );
	...
	function construyeCadenaFiltro() {
		...
		$res = $this->getSQLBusqueda( "es_jerarquia", $this->es_jerarquia, $res);
		
		return $res;
	}

	function newInstance() {
		return new ClsAtributosJerarquia();
	}
	...
	function getArrayCampos() {
		$arr = array(
			...
			"es_jerarquia"=>$this->es_jerarquia
		);
		return $arr;
	}
	...
	function estableceCampo($key,$valor,$prefix) {
		...
		else if ($key==$prefix."es_jerarquia") $this->es_jerarquia = $valor;
	} 
