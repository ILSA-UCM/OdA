		...
		foreach($controlados as $key=>$value){		
			if(($value->value!= "")){ 
				...
				//Nombre del elemento controlado
						...
						$value->value=str_replace("xxjj", "+", $value->value); // 2.- alfredo 180613 
				
				if(strlen($value->value)>20){
					$cadena = substr($value->value,0,20)."..";//alfredo 180613   de 30, a 20
				} else {
					$cadena = $value->value;
				}
				...
				$identificadorCaja=$seccion->id."_".str_replace(" ","-", substr($value->value,0,20));// hasta 30  alfredo 180614
				?>
					<li>
						<a href="<?=$enlace?>">
							<span><?=$cadena?></span>
...
<?
	function imprimeCabeceraX($idsec,$nombre,$idpadre,$acum_navegacion,$margenLateral,$margenSuperior){
		...
		if($acum_navegacion != ""){
			...
			$identificadorCaja = $idsec."_".substr(array_pop($arr_nuevo),0,20);// hasta 30  alfredo 180614
		}else{
