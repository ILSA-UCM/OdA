...
<form name="form_generacion" action="do.php" method="POST"  style="display:inline;">
	...
	?>
	<? $recurso = $filas[$i];?>
	<table class="item" border="0" cellspacing="0" cellpadding="-100" width="500px">					
					<tr class="<?= $lsregistros ?>">
						...
						<td rowspan="10" colspan="2" align="right" rowspan="10">
						 ...
						 ?>
									<a href="../bo/download/<?=$id?>/<?=$recurso->name?>" target="_blank">
										<div style="width:75px; overflow-y:hidden;">
											<img src="../bo/download/<?=$id?>/<?=$recurso->name?>" width="75px" border="0">
										</div>			
									</a>
								<? } else if (substr($recurso->name,strlen($recurso->name)-4, strlen($recurso->name))==".pdf"){ ?>
								<div class="imagen_recurso">
									<a href="../bo/download/<?=$id?>/<?=$recurso->name?>" target="_blank"><IMG SRC='img/icon_pdf.gif' WIDTH="50" HEIGHT="50" BORDER="0" ALT=""></a>
								</div>
								...
								<? } else if ((substr($recurso->name,strlen($recurso->name)-4, strlen($recurso->name))==".doc")||
									...
									<a href="../bo/download/<?=$id?>/<?=$recurso->name?>" target="_blank"><IMG SRC='img/icon_word.gif' WIDTH="50" HEIGHT="50" BORDER="0" ALT=""></a>
								<? } else { ?>
									<a href="../bo/download/<?=$id?>/<?=$recurso->name?>" target="_blank"><IMG SRC='img/icon_text.gif' WIDTH="37" HEIGHT="47" BORDER="0" ALT=""></a>
								<? } ?>		
							<? } else if ($recurso->type=="F"){
									if ($visit->util->esImagen($recurso->name)) { ?>
										<a href="../bo/download/<?=$recurso->idov_refered //alfredo 140728 ?>/<?=$recurso->name?>" target="_blank">
											<div style="width:75px; overflow-y:hidden;">
												<IMG SRC='../bo/download/<?=$recurso->idov_refered //alfredo 140728?>/<?=$recurso->name?>' WIDTH="75" BORDER="0" ALT="">
											</div>
										</a>
									<? } else if (substr($recurso->name,strlen($recurso->name)-4, strlen($recurso->name))==".pdf"){ ?>
									<div class="imagen_recurso">
								...
								<? } else if ((substr($recurso->name,strlen($recurso->name)-4, strlen($recurso->name))==".doc")||
							                  (substr($recurso->name,strlen($recurso->name)-5, strlen($recurso->name))==".docx")){ //alfredo 1400719 ?>
										<a href="../bo/download/<?=$id?>/<?=$recurso->name?>" target="_blank"><IMG SRC='img/icon_word.gif' WIDTH="50" HEIGHT="50" BORDER="0" ALT=""></a>
									<? } else { ?>
										<a href="../bo/download/<?=$id?>/<?=$recurso->name?>" target="_blank"><IMG SRC='img/icon_text.gif' WIDTH="37" HEIGHT="47" BORDER="0" ALT=""></a>
									<? } ?>
							...
							<? } else { ?>
								<a href="#"  onclick='window.open("cm_view_virtual_object.php?idov=<?=  $recurso->idov_refered ?>&seleccion=1","","width=700,height=650,scrollbars=yes"); return false;'>
									<div style="width:75px; overflow-y:hidden;">
										<? 
